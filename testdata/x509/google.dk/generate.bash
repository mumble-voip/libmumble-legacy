...
echo "" | openssl pkcs12 -export -nodes -nokeys -password stdin -out \*.google.dk-chain.p12 -in \*.google.dk-chain.pem

# Generate PKCS12 file with password 'password' for testing X509Certificate::FromPKCS12
echo "password" | openssl pkcs12 -export -nodes -nokeys -password stdin -out \*.google.dk-chain-password.p12 -in \*.google.dk-chain.pem

# Generate DER for leaf cert to test regular import, as well as other per-cert methods.
openssl x509 -in \*.google.dk-chain.pem -inform PEM -out \*.google.dk-leaf.crt -outform DER

# Generate SHA1 and SHA256 digests for leaf
cat \*.google.dk-leaf.crt | openssl sha -sha1 -binary > \*.google.dk-leaf.sha1
cat \*.google.dk-leaf.crt | openssl sha -sha256 -binary > \*.google.dk-leaf.sha256
...
openssl x509 -in \*.google.dk-leaf.crt -inform DER -noout -startdate | sed 's,notBefore=,,' | mumble_tconv > \*.google.dk-leaf.notBefore
openssl x509 -in \*.google.dk-leaf.crt -inform DER -noout -enddate | sed 's,notAfter=,,' | mumble_tconv > \*.google.dk-leaf.notAfter
